...
export default function ChallengeGrid({ challenges, isNavigating, onChallengeSelect }: ChallengeGridProps) {
	return (
		<div className="flex-1 w-full px-2 overflow-y-hidden flex flex-col items-center" style={{ minHeight: '50vh' }}>
			<div className="w-full max-w-2xl grid grid-cols-4 grid-rows-3 gap-0 h-full justify-items-center">
				{challenges.map((challenge, index) => {
					return (
						<div
							key={challenge._id}
							onClick={() => onChallengeSelect(challenge._id)}
						...
						>
							...
							<div className="absolute top-1 left-1/2 -translate-x-1/2 flex items-center justify-center w-full">
								<span className="w-full text-center text-black font-bold drop-shadow-lg text-[240%] symbol-font">
									{challenge.symbol}
								</span>
							</div>
							{/* Estrellas en la parte inferior */}
							{challenge.stars > 0 && (
								<div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex gap-0.5 -ml-2">
